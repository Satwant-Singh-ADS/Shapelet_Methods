...
#     usc = axs[2].set(xlabel="Week", ylabel="Cosine Similarity",label = "USC Shapelet Ensemble")
    ...
    axs.set_title("Model Evaluation for Shapelet Ensemble and Hub Ensemble over time for State {}".format(state),fontsize=23)
#         plt.show()

    subset = Master_df_actual_VS_Model_Agrrement[Master_df_actual_VS_Model_Agrrement['State']==state]
    subset_1 = subset[subset['Model Name']==ensemble_model_name]
    ...
    ensemble  = axs.plot(x,y,label="Hub Ensemble")
...
covid_ensemble = axs.scatter(ag_score, perf_score_ensemble,label="Hub Ensemble")
